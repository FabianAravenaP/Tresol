...
async function run() {
  try {
    ...
    // Find the most recent based on fecha_ingreso (today is 2026-04-02)
    const today = '2026-04-02';
    const recent = missing.filter(p => p.fecha_ingreso === today || p.fecha_ingreso === '2026-04-01');
    
    fs.writeFileSync('to_import.json', JSON.stringify({ totalMissing: missing.length, todayNew: recent.length, recent }, null, 2));
    console.log('Detected', missing.length, 'missing in total.', recent.length, 'from today/yesterday.');
  } catch (err) {
    console.error(err);
  }
}
