...
export default function Home() {
  ...
  // loginStep: 'choose' → tile selection  |  'auth' → user search + password
  const [loginStep, setLoginStep] = useState<'choose' | 'auth'>('choose')
  const [loginType, setLoginType] = useState<'usuario' | 'admin'>('usuario')
    ...
    try {
      const [resUsers, resPersonas] = await Promise.all([
        fetch('/api/proxy', {
          ...
          body: JSON.stringify({ table: 'usuarios', method: 'select', data: 'id, nombre, rol, password, rut' })
        }),
        fetch('/api/proxy', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ table: 'maestro_personas', method: 'select', data: 'rut, cargo, id' })
        })
      ])

      if (!resUsers.ok || !resPersonas.ok) throw new Error('Error al cargar datos desde el proxy')
      ...
      const cargoMap = (personasData || []).reduce((acc: any, p: any) => {
        if (p.rut) acc[p.rut] = { cargo: p.cargo || '', id: p.id }
        return acc
      }, {})

      setUsuarios((usersData || []).map((u: any) => ({
        ...u,
        displayCargo: cargoMap[u.rut]?.cargo || u.rol,
        persona_id: cargoMap[u.rut]?.id || null
      })))
    } catch (error) {
  ...
  useEffect(() => {
    if (isLoginOpen && loginStep === 'auth') {
      fetchUsuarios()
    }
  }, [isLoginOpen, loginStep])

  const handleLogin = (user: any) => {
    const requiresPassword = Boolean(user.password)
  ...
  // Admin tile pre-filters to users with passwords or admin/gerente/jefe roles
  const filteredUsers = usuarios.filter(u => {
    ...
    if (loginType === 'admin') {
      const roleUp = (u.rol || '').toUpperCase()
      const isAdmin =
        roleUp.includes('ADMIN') ||
        roleUp.includes('GERENTE') ||
        roleUp.includes('JEFE') ||
        Boolean(u.password)
        ...
        <section id="soluciones" className="py-24 bg-zinc-50/50 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { title: "Cumplimiento Normativo", desc: "Digitalización de certificados, sellos y reportes automáticos para cumplir con la ley ambiental vigente sin papeleo manual.", icon: ShieldCheck, color: "#51872E" },
                { title: "Optimización de Costos", desc: "Reduce el consumo de combustible y desgaste de flota mediante inteligencia aplicada en la planificación de rutas diarias.", icon: TrendingDown, color: "#116CA2" },
                { title: "Trazabilidad Inteligente", desc: "Visibilidad total del residuo desde su origen hasta la disposición final, con evidencias fotográficas y geolocalización.", icon: Target, color: "#FBC15F" }
              ].map((sol, i) => (
                <div key={i} className="group p-10 rounded-[2.5rem] bg-white border border-zinc-200/50 hover:border-[#116CA2]/30 hover:shadow-2xl transition-all duration-500">
                  ...
                  <p className="text-zinc-500 font-medium leading-relaxed text-sm">{sol.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="modulos" className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { title: "Gestión Cocina", desc: "Control de minutas, producción diaria e inventario de casino para el personal.", icon: ChefHat, color: "#FBC15F" },
                { title: "Operaciones", desc: "Planificación de servicios en calendario, asignación de choferes y control de flota.", icon: LayoutDashboard, color: "#116CA2" },
                { title: "App Conductor", desc: "Terminal móvil para reportar inicio de ruta, carga y generación de comprobantes digitales.", icon: Smartphone, color: "#51872E" },
                { title: "Digitalizador", desc: "Estación de visualización y exportación de certificados oficiales y reportes operativos.", icon: ClipboardCheck, color: "#FBC15F" },
                { title: "Control Portería", desc: "Registro de ingresos y salidas para porterías Husamontt y Tresol Antiguo.", icon: Shield, color: "#116CA2" },
                { title: "Préstamo Vehículo", desc: "Módulo para la solicitud, retiro y entrega de vehículos menores por parte del personal.", icon: Car, color: "#51872E" },
                { title: "Gestión Activos", desc: "Control, ubicación y estado de contenedores y otros activos de la empresa.", icon: Package, color: "#51872E" }
              ].map((mod, i) => (
                <Card key={i} className="group hover:shadow-2xl transition-all duration-500 border-none ring-1 ring-black/5 rounded-[2.5rem] overflow-hidden bg-zinc-50/50 hover:bg-white cursor-pointer" onClick={openLogin}>
                  <CardContent className="p-10 space-y-6">
                    ...
                    <div className="space-y-3 font-heading">
                      ...
                      <p className="text-zinc-500 font-medium font-sans leading-relaxed text-sm">{mod.desc}</p>
                    </div>
        ...
        <DialogContent className="sm:max-w-lg rounded-[3rem] border-none shadow-2xl p-0 overflow-hidden bg-white/95 backdrop-blur-xl">
            ...
            <DialogDescription className="text-white/70 font-bold">
              {loginStep === 'choose'
                ...
                : loginType === 'admin'
                  ...
                  : "Busca tu nombre en el directorio."}
            </DialogDescription>
          </div>
          ...
          {loginStep === 'choose' && (
            <div className="p-10 space-y-4">
              <button
                onClick={() => { setLoginType('usuario'); setLoginStep('auth') }}
                className="w-full flex items-center gap-5 p-6 rounded-2xl bg-slate-50 hover:bg-[#51872E]/5 border-2 border-transparent hover:border-[#51872E]/30 transition-all group text-left"
              >
                <div className="size-14 rounded-2xl bg-[#51872E]/10 flex items-center justify-center group-hover:bg-[#51872E] transition-colors shrink-0">
                  <Users className="size-7 text-[#51872E] group-hover:text-white transition-colors" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-[#323232] uppercase tracking-tight text-lg">Usuario</p>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Personal operativo y de servicios</p>
                </div>
                <ArrowRight className="size-5 text-slate-300 group-hover:text-[#51872E] transition-colors shrink-0" />
              </button>

              <button
                onClick={() => { setLoginType('admin'); setLoginStep('auth') }}
                className="w-full flex items-center gap-5 p-6 rounded-2xl bg-slate-50 hover:bg-[#116CA2]/5 border-2 border-transparent hover:border-[#116CA2]/30 transition-all group text-left"
              >
                <div className="size-14 rounded-2xl bg-[#116CA2]/10 flex items-center justify-center group-hover:bg-[#116CA2] transition-colors shrink-0">
                  <Shield className="size-7 text-[#116CA2] group-hover:text-white transition-colors" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-black text-[#323232] uppercase tracking-tight text-lg">Administrador</p>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Gestión, supervisión y control</p>
                </div>
                <ArrowRight className="size-5 text-slate-300 group-hover:text-[#116CA2] transition-colors shrink-0" />
              </button>
            ...
            <div className="p-10 space-y-6">
              {!selectedUser ? (
                <>
                    ...
                    {isLoading ? (
                      ...
                      >
                        <div className="flex items-center gap-4 text-left">
                          <div className="size-12 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-[#116CA2] group-hover:bg-[#116CA2] group-hover:text-white transition-all shrink-0">
                  ...
                  <Button
                    variant="ghost"
                    className="w-full h-10 rounded-xl font-black text-slate-400 uppercase tracking-widest text-xs"
                    onClick={() => { setLoginStep('choose'); setSearchTerm("") }}
                  >
                    ← Volver
                  </Button>
                </>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="size-12 rounded-xl bg-[#116CA2] text-white flex items-center justify-center font-black shrink-0">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter') handleLogin(selectedUser) }}
                    />
                    {passwordError && (
